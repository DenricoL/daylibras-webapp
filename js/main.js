...
var wordList = ["anjos", "arroz", "aviao", "beber", "bolsa", "carro", "carta", "chave", "comer", "deusa", "gaita", "garfo", "grato", "justo", "lento", "marca", "nadar", "palco", "peixe", "pente", "piano", "porco", "porta", "pouco", "prato", "pular", "raiva", "risos", "sagaz", "saude", "senso", "serra", "vigor", "vozes"];
...
// Escolhe a palavra correspondente
const word = wordList[diasPassados % wordList.length].toUpperCase();

// Mostra a palavra do dia
...
// Função que altera as estatísticas da página de perfil
// Atualiza estatísticas do usuário
function incrementCorrectAnswers(rodadaPerfeita = false) 
{
  fetch("incrementCorrectAnswers", {
    method: "POST",
    headers: { "Content-Type": "application/x-www-form-urlencoded" },
    body: `rodadaPerfeita=${rodadaPerfeita}`
  })
  .then(response => {
    if (!response.ok) throw new Error("Erro ao atualizar acertos");
    return response.json();
  })
  .then(data => {
    console.log("Estatísticas atualizadas:", data);

    const total = document.getElementById("totalAcertos");
    const perfeitos = document.getElementById("acertosPerfeitos");
    const sequencia = document.getElementById("maiorSequencia");
    const nivel = document.getElementById("nivel");

    if (total) total.innerText = data.totalAcertos;
    if (perfeitos) perfeitos.innerText = data.acertosPerfeitos;
    if (sequencia) sequencia.innerText = data.maiorSequencia;
    if (nivel) nivel.innerText = data.nivel;
  })
  .catch(err => console.error("Erro ao incrementar acertos:", err));
}

// Reseta a sequência quando o jogador erra
function wrongAnswer() 
{
  fetch("wrongAnswer", { method: "POST" })
    .then(response => response.json())
    .then(data => {
      console.log("Sequência resetada:", data);
      const sequencia = document.getElementById("maiorSequencia");
      if (sequencia) sequencia.innerText = data.sequenciaAtual || 0;
    })
    .catch(err => console.error("Erro ao resetar sequência:", err));
}
...
function processInput(e) 
{
	...
	if (!gameOver && row == height) {
	    ...
	    // Resetar sequência de acertos no servidor
	    wrongAnswer();
	}
...
{
	document.getElementById('popupTutorial').style.display = 'block';	
}
